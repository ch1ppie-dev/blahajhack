...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (this.itemToRender != null)
        {
            ...
            final int itemInUseCount = entityplayersp.getItemInUseCount();
            boolean usingItem = itemInUseCount > 0;

            EventItemRender event = new EventItemRender(action, usingItem, animationProgression, partialTicks, swingProgress, itemToRender);
            Client.INSTANCE.getBus().post(event);

            usingItem = event.isUsingItem();
            animationProgression = event.getAnimationProgression();
            swingProgress = event.getSwingProgress();

            if (this.itemToRender.getItem() instanceof ItemMap)
            {
                this.renderItemMap(entityplayersp, f2, animationProgression, swingProgress);
            }
            else if (usingItem)
            {
        ...
        {
            this.func_178095_a(entityplayersp, animationProgression, swingProgress);
        }
